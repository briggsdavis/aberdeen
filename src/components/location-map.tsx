...
const mapViewport = { height: 600, width: 400, x: 200, y: 0 }
const mapViewBox = `${mapViewport.x} ${mapViewport.y} ${mapViewport.width} ${mapViewport.height}`
// Third row, second column after the map is rotated.
const aberdeenBuildingPoint = { x: 435, y: 432 }
const locationStarStyle = {
  left: `${((aberdeenBuildingPoint.x - mapViewport.x) / mapViewport.width) * 100}%`,
  top: `${((aberdeenBuildingPoint.y - mapViewport.y) / mapViewport.height) * 100}%`,
}
  ...
  return (
    ...
    >
      <div className="relative grid h-12 w-12 place-items-center">
        ...
        <span className="absolute top-1/2 left-11 -translate-y-1/2 -rotate-90 border border-aberdeen-blue bg-oyster-white px-3 py-2 font-utility text-xs tracking-[0.16em] whitespace-nowrap text-aberdeen-blue uppercase shadow-[4px_4px_0_var(--color-citrus)] md:left-12">
          Aberdeen
        </span>
      </div>
    </motion.div>
...
export function LocationMap({ className = "", location }: LocationMapProps) {
  ...
  return (
    <a
      ...
      className={`group relative block aspect-3/2 overflow-hidden bg-oyster-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-citrus ${className}`}
      href={placeUrl}
      rel="noreferrer"
      target="_blank"
    >
      <div className="absolute top-1/2 left-1/2 h-[150%] w-2/3 -translate-x-1/2 -translate-y-1/2 rotate-90">
        <svg
          aria-hidden="true"
        ...
        >
          ...
          <g fill="var(--color-aberdeen-blue)">
            ...
            <path d="m151 469 61 9-3 28-65-10Z" />
            <path d="m278 491 60 5 1 27-63-4Z" />
            <path d="m408 500 64 1 3 27-68-2Z" />
            <path d="m535 498 48-4 5 28-51 4Z" />
            <path d="m633 495 20-3-8 27-18 1Z" />
          </g>
